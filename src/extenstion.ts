...
import * as functionParser from './parser';
export function activate(ctx:vscode.ExtensionContext) {
	...
	vscode.commands.registerCommand('extension.addDocComments', () => {
		...
		if ((lang == "typescript") || (lang == 'javascript')) {
			...
			const outputMessage: string = 'Please select a function signature'
			...
			let returnText = functionParser.getReturns(selectedText);
			...
			if (params.length > 0) {
				...
				vscode.window.activeTextEditor.edit((editBuilder: vscode.TextEditorEdit) => {
					...
					const line:string = vscode.window.activeTextEditor.document.lineAt(selection.start.line).text;
